...
class ImportLegacyClientAccounts extends Command
{
    ...
    public function handle(): int
    {
        ...
        $query->chunk(100, function ($chunk) use (&$imported, &$skipped, &$errors, $dryRun, $bar) {
            foreach ($chunk as $account) {
                $bar->advance();

                $email = strtolower(trim($account->email ?? ''));

                // Skip accounts without valid email
                if (empty($email) || ! filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    $skipped++;
                    continue;
                }

                // Skip if already imported by legacy_client_account_id
                if (Customer::where('legacy_client_account_id', $account->id)->exists()) {
                    $skipped++;
                    continue;
                }
        ...
        $bar->finish();
        $this->newLine(2);
        $this->info("Done: {$imported} created, {$skipped} skipped, {$errors} errors");
