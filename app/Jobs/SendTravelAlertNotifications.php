...
class SendTravelAlertNotifications implements ShouldQueue, ShouldBeUnique
{
    ...
    {
        // Nur Travel-Alert-Regeln auslösen (GTM hat eigenen Schedule/Button)
        $sentCount = match (true) {
            $this->event instanceof CustomEvent => $service->processCustomEvent($this->event, $this->force, 'travel-alert'),
